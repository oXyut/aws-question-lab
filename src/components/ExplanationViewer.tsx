...
function RequirementText({
  ...
  return (
    <div className="question-text" aria-label="問題文">
...
export function ExplanationViewer({
  ...
  return (
    <div className="explanation-viewer studio-viewer">
      <section className="studio-question" aria-label="問題と要件">
      ...
      <section className="answer-overview" aria-label="解答のポイント">
        ...
        <p className="answer-rationale">{explanation.answerRationale}</p>
        {hasUnverified && (
        ...
        <div className="answer-learning">
          <h3>
            <Zap size={17} /> 正解を決める要点
          </h3>
      ...
      <section className="studio-comparison" aria-label="要件の判断と選択肢">
        <div className="comparison-intro">
          <h2>要件から、選択肢を比べる</h2>
          <p>要件を選ぶと評価が連動します。選択肢を選んで、その構成と根拠を確認できます。</p>
        </div>
        <div className="requirement-controls">
          ...
          <nav className="studio-stepbar" aria-label="要件を順に確認">
            <span className="stepbar-title">
              <ListFilter size={18} />
              {requirementStep >= 0
                ? `${requirementStep + 1} / ${explanation.requirements.length} の要件まで評価`
                : '要件を順に見る'}
            </span>
            <button
        ...
        <div className="studio-evaluations">
          {explanation.evaluations.map((evaluation) => {
            ...
            const title =
              architecture?.title.replace(/^[A-ZＡ-Ｚa-z0-9]+\s*[·.：:、-]\s*/, '') ?? option.text;
            const isSelected =
            ...
            return (
              ...
              >
                <button
                  ...
                  aria-controls={`architecture-${revision.id}`}
                  onClick={() => selectOption(option.id)}
                >
                  <span className="option-letter">{option.label}</span>
        ...
        <section className="studio-diagram" id={`architecture-${revision.id}`} aria-label="構成図">
      ...
      <div className="study-details">
        <details className="sources-section" id="evidence-details">
          <summary>
            <ShieldCheck size={18} />
            <strong>AWS公式資料・引用</strong>
            <span>
              {explanation.sources.length}件
              {hasUnverified ? ' · 要確認あり' : ' · 公式本文と照合済み'}
            </span>
        ...
        {!!explanation.glossary.length && (
          <details className="glossary-section">
            <summary>
              <BookOpen size={18} />
              <strong>用語を押さえる</strong>
              <span>{explanation.glossary.length}語</span>
              <ChevronDown size={16} className="disclosure-chevron" />
            </summary>
