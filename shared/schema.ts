...
export const QuestionDraftSchema = z.object({
  ...
  options: z
    .array(z.object({ id: Id, label: z.string().max(20), text: z.string().min(1).max(5000) }))
    .max(12),
  ...
  edges: z
    .array(
      z.object({
        ...
        to: Id,
        label: z.string(),
        kind: z.enum(['data', 'metadata', 'control']),
        requirementIds: z.array(Id),
      }),
    )
    .max(60),
...
export function validateQuestion(question: QuestionDraft): void {
  const options = new Set(question.options.map((option) => option.id));
  if (options.size !== question.options.length) throw new Error('選択肢のIDが重複しています');
  if (
    new Set(question.knownAnswerIds).size !== question.knownAnswerIds.length ||
    question.knownAnswerIds.some((id) => !options.has(id))
  )
    throw new Error('入力された正解の参照が正しくありません');
  if (question.selectionMode === 'none') {
    if (options.size || question.knownAnswerIds.length || question.selectionCount !== null)
      throw new Error('自由記述の問題に選択肢・選択数を指定できません');
  } else {
    if (
      !options.size ||
      (question.selectionCount !== null && question.selectionCount > options.size)
    )
      throw new Error('選択肢の数と選択数が一致しません');
    if (
      question.selectionMode === 'single' &&
      question.selectionCount !== null &&
      question.selectionCount !== 1
    )
      throw new Error('単一選択の選択数は1です');
  }
}
...
export function validateReferences(
  ...
  if (question.selectionMode === 'single' && explanation.answerOptionIds.length > 1)
    throw new Error('単一選択の解答が複数あります');
  if (
    question.selectionCount &&
    explanation.answerOptionIds.length &&
    explanation.answerOptionIds.length !== question.selectionCount
  )
    throw new Error('指定された解答の選択数と一致しません');
  if (question.selectionMode === 'none' && question.options.length)
    throw new Error('自由記述の問題に選択肢があります');
  const reqs = unique(
    explanation.requirements.map((r) => r.id),
    '要件',
  );
  explanation.requirements.forEach((r) => {
    if (!quoteSpan(question.text, r))
      throw new Error(`要件「${r.label}」の引用が問題文にありません`);
  });
  const sources = unique(
    explanation.sources.map((s) => s.id),
    '出典',
  );
  const graphs = unique(
    explanation.architectures.map((g) => g.id),
    '構成図',
  );
  requireIds([explanation.recommendedArchitectureId], graphs, '推奨構成図');
  const nodeIds = unique(
    explanation.architectures.flatMap((g) => g.nodes.map((n) => n.id)),
    'ノード',
  );
  const edgeIds = unique(
    explanation.architectures.flatMap((g) => g.edges.map((e) => e.id)),
    '接続線',
  );
  for (const graph of explanation.architectures) {
    ...
    graph.edges.forEach((e) => {
      requireIds(
        [e.from, e.to].filter((x, i, a) => a.indexOf(x) === i),
        graphNodes,
        '接続線の端点',
      );
    ...
    graph.steps.forEach((s) => {
      requireIds(s.nodeIds, graphNodes, 'ステップのノード');
      requireIds(s.edgeIds, graphEdges, 'ステップの接続線');
    });
  }
  const evaluated = unique(
    explanation.evaluations.map((e) => e.optionId),
    '評価',
  );
  if (evaluated.size !== options.size || [...evaluated].some((id) => !options.has(id)))
    throw new Error('全選択肢の評価が必要です');
  for (const evaluation of explanation.evaluations) {
    ...
    const evaluationGraph = explanation.architectures.find(
      (g) => g.id === evaluation.architectureId,
    );
    if (evaluationGraph && !evaluationGraph.optionIds.includes(evaluation.optionId))
      throw new Error('評価の構成図が対応する選択肢を含んでいません');
    unique(
      evaluation.checks.map((c) => c.requirementId),
      '要件の評価',
    );
    const checkedRequirements = new Set(evaluation.checks.map((c) => c.requirementId));
    if (
      explanation.requirements.some((r) => r.kind !== 'context' && !checkedRequirements.has(r.id))
    )
      throw new Error('すべての必須・比較要件について選択肢を評価してください');
    for (const check of evaluation.checks) {
      ...
      if (evaluationGraph) {
        requireIds(
          check.nodeIds,
          new Set(evaluationGraph.nodes.map((n) => n.id)),
          '対応構成図のノード',
        );
        requireIds(
          check.edgeIds,
          new Set(evaluationGraph.edges.map((e) => e.id)),
          '対応構成図の接続線',
        );
      }
...
export function applyEvidenceStatus(explanation: Explanation): Explanation {
  const verified = new Set(
    explanation.sources.filter((s) => s.status === 'verified').map((s) => s.id),
  );
  const contextIds = new Set(
    explanation.requirements.filter((r) => r.kind === 'context').map((r) => r.id),
  );
  return {
    ...explanation,
    evaluations: explanation.evaluations.map((e) => {
      const checks = e.checks.map((c) =>
        !contextIds.has(c.requirementId) &&
        c.verdict !== 'unknown' &&
        (!c.sourceIds.length || c.sourceIds.some((id) => !verified.has(id)))
          ? { ...c, verdict: 'unknown' as const, reason: `要確認：${c.reason}` }
          : c,
      );
      const decisions = checks.filter((c) => !contextIds.has(c.requirementId));
      const overall: Verdict = decisions.some((c) => c.verdict === 'violates')
        ? 'violates'
        : decisions.some((c) => c.verdict === 'unknown') || !decisions.length
          ? 'unknown'
          : decisions.some((c) => c.verdict === 'inferior')
            ? 'inferior'
            : 'meets';
