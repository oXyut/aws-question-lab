...
import { z } from 'zod';
import {
  ExplanationInputSchema, QuestionDraftSchema, applyEvidenceStatus, quoteSpan,
  validateReferences, type Explanation, type QuestionDraft,
} from '../shared/schema';

function fixture(): { question: QuestionDraft; explanation: Explanation } {
  return {
    question: { title: '選択の根拠', text: 'データを再処理できること。運用負荷を抑える。',
      options: [{ id: 'A', label: 'A', text: '保持する' }, { id: 'B', label: 'B', text: '捨てる' }],
      selectionMode: 'single', selectionCount: 1, knownAnswerIds: [], originalExplanation: '', uncertainties: [], imageIds: [] },
    explanation: {
      title: '再処理と保持', shortAnswer: 'A', answerOptionIds: ['A'], answerRationale: '保持が必要', assumptions: [],
      requirements: [{ id: 'r1', label: '再処理', quote: '再処理できる', quoteOccurrence: 0, kind: 'hard', explanation: '読み直せる必要があります' }],
      evaluations: ['A', 'B'].map((id) => ({ optionId: id, overall: id === 'A' ? 'meets' : 'violates', summary: '判断', conditionsToBeCorrect: '再処理の要件がない場合', architectureId: 'g1',
        checks: [{ requirementId: 'r1', verdict: id === 'A' ? 'meets' : 'violates', reason: '保持の有無', sourceIds: ['s1'], nodeIds: ['n1'], edgeIds: [] }] })),
      architectures: [{ id: 'g1', title: '構成', optionIds: ['A'], description: '保持', nodes: [{ id: 'n1', service: 's3', label: 'S3', description: '保存', requirementIds: ['r1'] }], edges: [], steps: [{ title: '保存', description: '保存する', nodeIds: ['n1'], edgeIds: [] }] }],
      recommendedArchitectureId: 'g1', learningPoints: ['保持と配信を分ける'], glossary: [],
      sources: [{ id: 's1', title: '仕様', url: 'https://docs.aws.amazon.com/AmazonS3/latest/userguide/Welcome.html', excerpt: 'Amazon S3', status: 'verified', checkedAt: '2026-09-23T00:00:00.000Z', verificationNote: '引用本文を照合' }], caveats: [],
    },
  };
}
...
test('broken edge endpoints and step links are rejected', () => {
  const { question, explanation } = fixture();
  explanation.architectures[0].edges.push({ id: 'e1', from: 'n1', to: 'missing', label: '転送', kind: 'data', requirementIds: [] });
...
test('a verified hard violation remains a violation even with other unknown checks', () => {
  const { explanation } = fixture();
  explanation.evaluations[1].checks.push({ ...explanation.evaluations[1].checks[0], requirementId: 'other', sourceIds: [], verdict: 'unknown' });
...
test('multiple selection count and known-answer references are checked', () => {
  const { question, explanation } = fixture();
  question.selectionMode = 'multiple'; question.selectionCount = 2;
...
test('open-ended questions have no fabricated options or answers', () => {
  const { question, explanation } = fixture();
  question.options = []; question.selectionMode = 'none'; question.selectionCount = null;
  explanation.answerOptionIds = []; explanation.evaluations = []; explanation.architectures[0].optionIds = [];
  assert.doesNotThrow(() => validateReferences(question, explanation));
});
